...
st.header("PROCESAMIENTO ANALÍTICO, KPI, TENDENCIAS Y PREDICCIONES Juan 2024")
...
def Home():
    with st.expander("Ver Datos en Excel"):
        showData=st.multiselect('Filtrar Columna: ',df_selection.columns,default=["Policy","Expiry","Location","State","Region","Investment","Construction","BusinessType","Earthquake","Flood","Rating"])
    ...
    with total1:
        st.info('Suma Total Inversion',icon="💰")
        st.metric(label="Suma Total",value=f"{total_investment:,.0f}")
    ...
    with st.expander("DISTRIBUTIONS Por Frecuencia"):
     df.hist(figsize=(16,8),color='green', zorder=2, rwidth=0.9,legend = ['Investment'])
     st.pyplot()
...
st.subheader('CARACTERÍSTICAS PARA EXPLORAR LAS TENDENCIAS DE DISTRIBUCIÓN POR CUARTILES',)
...
fig2 = go.Figure(
    ...
    layout=go.Layout(
        title=go.layout.Title(text="TIPO DE NEGOCIO POR INVERSIÓN"),
